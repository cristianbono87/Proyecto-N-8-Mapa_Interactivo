  ...
  function usaDireccion (direccion, funcionALlamar) {
    ...
    geocodificador.geocode({ address: direccion }, function(results, status) {
      if (status == 'OK') {
        ...
        alert(
          'Geocode no fue exitoso, por la siguiente razón: ' +
            status +
            '. Si no obtienes resultados, intenta colocar el nombre del lugar y no la dirección'
        );
      }
    });
  }
